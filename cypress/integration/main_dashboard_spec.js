...
describe('Main dashboard flow', () => {
  ...
  it('should display the header', () => {
    ...
    // cy.intercept('https://spacelaunchnow.me/api/ll/2.2.0/launch/upcoming/?limit=20&mode=detailed', {
    //   upcomingLaunches: [
    //     {
    ...
    // cy.intercept('https://spacelaunchnow.me/api/ll/2.2.0/event/upcoming/?limit=20&mode=detailed', {
    //   upcomingEvents: [
    //     {
    //       id: 332,
  ...
  it('should display a list of the next 19 upcoming launches', () => {
    cy.visit('http://localhost:3000/')
    .get('.upcoming-header').contains('Upcoming Launches')
    .get('.upcoming-cards-container').find('.launch-card').should('have.length', 19)
  })

   it('should display launch cards that include details on the launch', () => {
    cy.visit('http://localhost:3000/')
    .get('.launch-card').first().find('img').should('have.attr', 'src').should('include', 'https://spacelaunchnow-prod-east.nyc3.cdn.digitaloceanspaces.com/media/launch_images/electron25202_image_20220420085230.jpg')
    .get('.launch-mission-description').contains('Commercial rideshare mission including payloads for Alba Orbital, Astrix Astronautics, Aurora Propulsion Technologies, E-Space, Spaceflight Inc, Swarm Technologies and UNSEENLABS.')
  })

  it('should display 20 upcoming events', () => {
    cy.visit('http://localhost:3000/')
    .get('.events-header').contains('Upcoming Events in Spaceflight')
    .get('.events-container').find('.event-card').should('have.length', 20)
  })
