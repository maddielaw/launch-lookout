...
const NextLaunch = () => {
  ...
  const addBookmark = () => {
    const checkedMission = nextLaunch.mission
    ...
    const newLaunch = {
      ...
      launchDate: format(
        new Date(nextLaunch.window_start),
        "E. MMMM dd, yyy, p"
      ),
    };
  ...
  useEffect(() => {
    if (
      bookmarkData.bookmarks.some((bookmark) => bookmark.id === nextLaunch.id)
    ) {
      setIsBookmarked(true);
    }
  }, []);
  ...
  return (
    <section className="next-launch-section">
      ...
      <p className="next-launch-date">
        {format(new Date(nextLaunch.window_start), "E. MMMM dd, yyy, p")}
      </p>
      <Countdown
        date={nextLaunch.window_start}
        status={nextLaunch.status.name}
        statusDescription={nextLaunch.status.description}
      />
